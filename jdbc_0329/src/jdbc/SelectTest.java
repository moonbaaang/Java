...
// 오라클 자동시작 - ojdbc6.jar 설치, 이클립스 세팅
	public static void main(String[] args) { // String... args 비정형 매개변수
		...
		try {
			...
			String sql = "select count(*) from c_emp group by dept_id"; //null값 포함 테이블 레코드 개수 리턴, 1행 1열 리턴
			st = conn.createStatement();
			// db로 전송하는 객체 - Statement의 도움을 받아야함, sql을 Statement 객체를 통해 conn으로 전송
			
			rs = st.executeQuery(sql);
			// 실행
			rs.next(); //1행 이동 > 결과에 따라 while문 사용할지 결정한다.
			System.out.println(rs.getInt("count(*)")); //컬럼명, index, 별칭, 함수명 가능
